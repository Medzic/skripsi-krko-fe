...
import ButtonAmbil from './ButtonAmbil';
...
const Dropdown = ({ data }) => {
    ...
    return (
      <div className="tabel-pengajuan" >
        {data.filter(pengajuan => pengajuan.noreg && pengajuan.picked === false).length > 0 ?
        ...
        {isOpen && (
          <Table className='tabel-pengajuan' striped bordered hover size="sm">
            ...
            <tbody>
              {data
                .filter(pengajuan => pengajuan.noreg && pengajuan.picked === false )
                .map((filteredPengajuan, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>{filteredPengajuan.namep1}</td>
                    <td className='btn-proses'>
                      <ButtonAmbil onSuccess={filteredPengajuan.id}/>
                    </td>
                  </tr>
                ))}
            </tbody>
...
export default TabelPengajuanAmbilAdmin